...
The tag is force-moved to the commit it was built from. The release notes name that commit and
list every PR merged since the newest `extender-v*` release, generated by GitHub itself so the
list matches what a real tagged release produces.
