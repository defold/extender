		...
		for(String platform : authenticatedPlatforms) {
			switch(platform) {
				...
				case "html5":
					...
					http.authorizeHttpRequests((authorizeHttpRequest) -> authorizeHttpRequest.requestMatchers("/build_async/wasm_pthread-web/**").hasRole("HTML5")).httpBasic(withDefaults());
					break;
				case "switch":
